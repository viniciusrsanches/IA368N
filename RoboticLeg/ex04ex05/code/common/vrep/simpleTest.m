...
function simpleTest()
  % Make sure to have the simulation scene mooc_exercise.ttt running in V-REP!
    ...
    vrep=connection.vrep;
        ...
        [res,objs]=simxGetObjects(connection.clientID,vrep.sim_handle_all,vrep.simx_opmode_blocking);
        ...
        startTime=t(6);
        currentTime=t(6);
        simxGetIntegerParameter(connection.clientID,vrep.sim_intparam_mouse_x,vrep.simx_opmode_streaming); % Initialize streaming
        while (currentTime-startTime < 5)   
            [returnCode,data]=simxGetIntegerParameter(connection.clientID,vrep.sim_intparam_mouse_x,vrep.simx_opmode_buffer); % Try to retrieve the streamed data
        ...
        simxAddStatusbarMessage(connection.clientID,'Hello V-REP!',vrep.simx_opmode_oneshot);
